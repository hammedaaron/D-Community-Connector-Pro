...
const UserCard: React.FC<UserCardProps> = ({ card, onEdit }) => {
  ...
  // Identify card owner role
  const isCardAdmin = card.userId.startsWith('admin-');
  const isCardDev = card.userId === 'dev-master-root';

  // Permission logic
  const canManage = isDev || isOwnCard || (isAdmin && !isCardDev);
  ...
  return (
    <div 
      ...
      className={`relative rounded-[2rem] p-6 transition-all duration-300 flex flex-col h-full border z-10 overflow-hidden ${
        ...
        isCardAdmin ? 'ring-2 ring-orange-500 border-orange-500/50 shadow-[0_0_15px_rgba(249,115,22,0.2)]' : ''
      } ${
        isFollowed 
          ? 'scale-[0.98] border-indigo-500/30' 
          : 'hover:-translate-y-1 hover:shadow-xl'
      } ${isCardDev ? 'mystic-green-hover' : isCardAdmin ? '' : 'hover:border-indigo-300'}`}
    >
      {isCardDev && <div className="star-dust"></div>}
      ...
      <div className="flex flex-col h-full relative z-20">
        <div className="flex items-start justify-between mb-4 overflow-hidden">
          <div className="flex items-center gap-4 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h3 className={`font-extrabold text-base leading-tight break-words whitespace-normal overflow-visible min-h-[1.5em] transition-colors ${isDark ? 'text-white' : 'text-slate-900'}`}>
                {card.displayName}
              </h3>
              <div className="flex flex-wrap gap-1.5 mt-2">
                {isCardAdmin && (
                  <span className="bg-orange-500 text-white text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter shadow-sm">
                    Admin
                  </span>
                )}
                {isMutual ? (
        ...
        <div className={`grid grid-cols-2 gap-2 mb-6 p-3 rounded-2xl border transition-colors ${isDark ? 'bg-slate-950/50 border-slate-800' : 'bg-slate-50 border-slate-100'}`}>
          <div className="text-center border-r border-slate-200 dark:border-slate-800">
            <p className={`text-[14px] font-black transition-colors ${isCardDev ? 'text-emerald-400' : isCardAdmin ? 'text-orange-500' : 'text-indigo-500'}`}>{stats.followers}</p>
            <p className="text-[7px] font-black uppercase tracking-widest text-slate-400">Followers</p>
          </div>
          <div className="text-center">
            <p className={`text-[14px] font-black transition-colors ${isCardDev ? 'text-emerald-400' : isCardAdmin ? 'text-orange-500' : 'text-indigo-500'}`}>{stats.following}</p>
        ...
        <div className="mt-auto space-y-3">
          <a 
            ...
            className={`flex items-center justify-center gap-2 w-full py-3 px-4 text-xs font-black rounded-2xl transition-all border ${
              hasVisitedProfile 
                ...
                : (isDark ? 'bg-slate-800 hover:bg-slate-700 text-white border-slate-700' : 'bg-slate-50 hover:bg-slate-100 text-slate-700 border-slate-200 shadow-sm')
            }`}
          >
          ...
          {!isOwnCard ? (
              ...
              className={`w-full flex items-center justify-center gap-2 py-3 px-4 rounded-2xl text-xs font-black transition-all shadow-lg border-2 ${
                isFollowed 
                  ...
                  : !hasVisitedProfile 
                    ...
                    : (isCardDev ? 'bg-emerald-600 border-emerald-500 hover:bg-emerald-700' : isCardAdmin ? 'bg-orange-500 border-orange-400 hover:bg-orange-600' : 'bg-indigo-600 border-indigo-500 hover:bg-indigo-700') + ' text-white'
              }`}
            >
            ...
            <div className={`w-full text-center py-3 px-4 text-[10px] font-black rounded-2xl uppercase tracking-widest border border-dashed transition-colors ${isDark ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400' : 'bg-indigo-50 border-indigo-200 text-indigo-600'}`}>
              Your Identity
            </div>
          )}
        </div>
