    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300">
      <div className={`w-full max-w-lg overflow-hidden transform transition-all rounded-t-[3rem] sm:rounded-[3rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] animate-in slide-in-from-bottom-full duration-500 ${isDark ? 'bg-slate-900 border border-slate-800' : 'bg-white border border-slate-100'}`}>
        <div className="px-10 py-8 flex items-center justify-between">
          <h3 className={`font-black text-2xl tracking-tighter ${isDark ? 'text-white' : 'text-slate-900'}`}>{isEnforced ? 'Profile Exists' : 'Establish Node'}</h3>
        ...
        <div className="px-10 pb-10 space-y-8">
          {isEnforced ? (
            <div className={`p-8 rounded-[2rem] border text-center space-y-4 ${isDark ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-100'}`}>
              ...
              <p className="text-sm font-medium text-slate-500">Each member is limited to one profile per community. Please edit your existing card instead.</p>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <input type="text" value={link} onChange={(e) => setLink(e.target.value)} placeholder="x.com/username" className={`w-full rounded-2xl px-6 py-4 text-base font-bold transition-all focus:ring-4 focus:ring-indigo-500/10 outline-none border ${isDark ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-600' : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400'}`} />
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            ...
            {!isEnforced && (
              <button onClick={handlePost} disabled={!name.trim() || !link.trim() || isOptimizing} className="flex-1 py-5 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-2xl shadow-2xl shadow-indigo-500/30 transition-all text-sm uppercase tracking-widest disabled:opacity-30">
