  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-all duration-500 ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'} flex-col lg:flex-row`}>
      ...
      <main className={`flex-1 flex flex-col h-full overflow-hidden z-10 relative ${activeTab !== 'community' && activeTab !== 'notifications' && activeTab !== 'profile' ? 'hidden lg:flex' : 'flex'}`}>
        {/* Responsive Header */}
        <header className={`px-4 lg:px-8 py-3 lg:py-4 flex items-center justify-between sticky top-0 z-40 transition-all ${isDark ? 'bg-slate-900/60' : 'bg-white/80'} backdrop-blur-xl border-b transition-colors ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
          <div className="flex items-center gap-3 lg:gap-8 flex-1">
            ...
            <h1 className={`text-lg lg:text-2xl font-black tracking-tighter truncate flex items-center gap-2 transition-colors ${isDark ? 'text-white' : 'text-slate-900'}`}>
          ...
          <div className="flex items-center gap-2 lg:gap-3">
            ...
            <button onClick={onOpenCreateProfile} disabled={isCreationDisabled || !selectedFolderId} className={`${isCreationDisabled || !selectedFolderId ? 'bg-slate-200 dark:bg-slate-800 text-slate-400 cursor-not-allowed border border-slate-300 dark:border-slate-700' : 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-500/20'} px-4 lg:px-5 py-2 lg:py-2.5 rounded-xl text-xs lg:text-sm font-black transition-all flex items-center gap-2`}>
              ...
              <span className="uppercase tracking-widest transition-colors">{!selectedFolderId ? 'Select' : isCreationDisabled ? 'Joined' : 'Join'}</span>
            </button>
          </div>
        </header>

        {isSearchOpen && (
          <div className={`sm:hidden px-4 py-3 border-b animate-in slide-in-from-top-4 duration-200 transition-colors ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
      ...
      {activeTab === 'profile' && (
        <div className={`lg:hidden fixed inset-0 z-[70] p-8 animate-in slide-in-from-right duration-300 transition-colors ${isDark ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'}`}>
           <header className="flex items-center justify-between mb-12">
              <h2 className="text-3xl font-black tracking-tighter uppercase">Member Portal</h2>
           ...
           <div className="space-y-8">
              <div className="p-8 rounded-[2.5rem] bg-indigo-600 text-white shadow-2xl flex items-center gap-6">
                 ...
                 <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest opacity-60">{currentUser?.role} Operative</p>
                 </div>
              </div>
           ...
           <div className="absolute bottom-12 left-8 right-8 text-center opacity-30">
              <p className="text-[10px] font-black uppercase tracking-[0.4em] transition-colors">Connector Standard v3.2.0</p>
           </div>
        </div>
      )}
