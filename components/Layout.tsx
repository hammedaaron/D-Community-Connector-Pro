...
const Layout: React.FC<LayoutProps> = ({ children, onOpenCreateProfile }) => {
  ...
  // FIX: Disable creation for regular users/admins in Universal folders
  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-all duration-500 ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'} flex-col lg:flex-row`}>
        ...
        <header className={`px-4 lg:px-8 py-4 flex items-center justify-between sticky top-0 z-40 transition-all ${isDark ? 'bg-slate-900/60' : 'bg-white/80'} backdrop-blur-xl border-b ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className={`${isCreationDisabled || !selectedFolderId ? 'bg-slate-200 dark:bg-slate-800 text-slate-400 cursor-not-allowed border border-slate-300 dark:border-slate-700' : 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-500/20'} px-5 py-2.5 rounded-xl text-sm font-black transition-all flex items-center gap-2`} 
              title={!selectedFolderId ? "Select a folder" : isSystemFolder && !isDev ? "Universal Community (Restricted)" : isCreationDisabled ? "Limit Reached" : "Join community"}
            >
              ...
              <span className="hidden sm:inline uppercase tracking-widest">
                {!selectedFolderId ? 'Select Folder' : isSystemFolder && !isDev ? 'Locked' : isCreationDisabled ? 'Joined' : 'Join'}
              </span>
            </button>
          </div>
        </header>
      ...
      <nav className={`lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 px-6 py-4 flex gap-10 items-center rounded-3xl z-40 transition-all shadow-2xl border ${isDark ? 'bg-slate-900/90 border-slate-700' : 'bg-white/90 border-slate-100'} backdrop-blur-xl`}>
        ...
        <button onClick={onOpenCreateProfile} disabled={isCreationDisabled || !selectedFolderId} className={`p-4 -mt-10 rounded-2xl text-white shadow-xl transition-all ${isCreationDisabled || !selectedFolderId ? 'bg-slate-400 opacity-50' : 'bg-indigo-600 shadow-indigo-500/40'} ${activeTab === 'community' && !isCreationDisabled && selectedFolderId ? 'scale-110' : ''}`}><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d={isCreationDisabled ? "M5 13l4 4L19 7" : "M12 4v16m8-8H4"} /></svg></button>
